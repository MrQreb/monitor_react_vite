...
import { GraficaProduccionDiariaColores } from "../components/GraficaProduccionDiariaColores";

export function ProduccionHoraColoresPage() {
  const connection = useSocketConnection()
  const produccionDiaria = useProduccionDiaria()
      ...
      <div className="flex justify-center text-center">
        <div>
          <h1 className="text-xl font-extrabold tracking-tight text-white">
            Objetivos de producción de las Lineas
          </h1>
        </div>
      </div>
      <section className="flex min-h-0 w-full flex-1 flex-col gap-4">
        <div className="flex-1 min-h-0">
          <GraficaProduccionDiariaColores datos={produccionDiaria.data ?? []} />
        </div>
      </section>
    </div>
  )
