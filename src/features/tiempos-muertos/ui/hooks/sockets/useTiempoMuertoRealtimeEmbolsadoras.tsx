...
import { TIEMPOS_MUERTOS_EMBOLSADO_KEY } from "../queries/useTiempoMuertoEmbolsado";
import { TiempoMuertoEmbolsadorasHub } from "@/features/tiempos-muertos/api/socket/hub/tiempo-muerto-hub-embolsadoras";
 ...
 */
export function useTiempoMuertoRealtimeEmbolsadoras(
  areaId: number,
  enabled: boolean,
) {
  ...
  useEffect(() => {
    ...
    const handleCreated = (
      ...
      queryClient.setQueryData(
        [TIEMPOS_MUERTOS_EMBOLSADO_KEY, areaId],
        (
    ...
    hub.on(
      "tiempo-muerto:embolsado-creado",
      handleCreated,
    );

    hub.on(
      "tiempo-muerto:embolsado-finalizado",
      handleFinished,
    );
